...
namespace :deploy do
  ...
  desc "Deploy to server with FTP"
  task :ftp_sync do
    system "lftp -u #{login},#{password} -e \"cd #{app_path}; mput ./dist/views/*.html; mkdir -p #{app_path}/images; cd #{app_path}/images; mput ./images/*; cd #{app_path}/javascripts; put ./dist/javascripts/illiad.js; cd #{app_path}/stylesheets; put ./dist/stylesheets/illiad.css; exit\" #{host}"
  end
